...
impl Block {
    ...
    fn bits_to_target(bits: u32) -> Hash256 {
        ...
        if exponent <= 3 {
            ...
            for (i, byte) in bytes.iter().enumerate().take(exponent) {
                target[i] = *byte;
            }
            for item in target.iter_mut().skip(exponent).take(32 - exponent) {
                *item = 0;
            }
        } else if exponent < 32 {
            // Normal case
            ...
            for item in target.iter_mut().take(32).skip(start + 3) {
                *item = 0;
            }
        }
